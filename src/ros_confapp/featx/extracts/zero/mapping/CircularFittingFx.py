...
class CircularFitting():
    def __init__(self):
        self.show_animation = False

    def circle_fitting(self, x, y):
        """
    ...
    def get_sample_points(self, cx, cy, cr, angle_reso):
        x, y, angle, r = [], [], [], []
    ...
    def Fit(self):
        ...
        while time <= simtime:
            ...
            x, y = self.get_sample_points(cx, cy, cr, angle_reso)

            ex, ey, er, error = self.circle_fitting(x, y)
            ...
            if self.show_animation:  # pragma: no cover
                ...
                plt.axis("equal")
                plt.plot(0.0, 0.0, "*r")
                self.plot_circle(cx, cy, cr)
                plt.plot(x, y, "xr")
                self.plot_circle(ex, ey, er, "-r")
                plt.pause(dt)
            else:
                return [ex, ey, er, error]
